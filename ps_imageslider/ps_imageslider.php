...
class Ps_ImageSlider extends Module implements WidgetInterface
{
    ...
    protected function installSamples()
    {
        ...
        for ($i = 1; $i <= 3; ++$i) {
            ...
            foreach ($languages as $language) {
                ...
                $slide->mobile_image[$language['id_lang']] = 'sample-'.$i.'.jpg';
            }
            $slide->add();
        }
    }
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            foreach ($languages as $language) {
                ...
                /* Uploads mobile image and sets slide */
                ...
                if (isset($_FILES['mobile_image_'.$language['id_lang']]) &&
                    ...
                    if ($error = ImageManager::validateUpload($_FILES['mobile_image_'.$language['id_lang']])) {
                        ...
                        return false;
                    } elseif (!ImageManager::resize($temp_name, __DIR__.'/images/'.$salt.'_'.$_FILES['mobile_image_'.$language['id_lang']]['name'], null, null, $type)) {
                        $errors[] = $this->displayError($this->getTranslator()->trans('An error occurred during the image upload process.', array(), 'Admin.Notifications.Error'));
                    }
                    if (isset($temp_name)) {
                        @unlink($temp_name);
    ...
    public function renderWidget($hookName = null, array $configuration = [])
    {
        ...
        $configuration['mobile'] = $this->context->isMobile();
        ...
        if (!$this->isCached($this->templateFile, $this->getCacheId($cache))) {
            $this->smarty->assign($this->getWidgetVariables($hookName, $configuration));
        }

        return $this->fetch($this->templateFile, $this->getCacheId($cache));
    }
        ...
        if (is_array($slides)) {
            foreach ($slides as &$slide) {
                if($configuration['mobile']){
                    ...
                    $slide['sizes'] = @getimagesize((__DIR__ . DIRECTORY_SEPARATOR . 'images' . DIRECTORY_SEPARATOR . $slide['mobile_image']));
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`, hssl.`mobile_image`
        ...
        foreach ($slides as &$slide) {
            $slide['mobile_image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'ps_imageslider/images/'.$slide['mobile_image']);
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT hssl.`image`,hssl.`mobile_image`, hssl.`id_lang`
            FROM '._DB_PREFIX_.'homeslider hs
            LEFT JOIN '._DB_PREFIX_.'homeslider_slides hss ON (hs.id_homeslider_slides = hss.id_homeslider_slides)
            LEFT JOIN '._DB_PREFIX_.'homeslider_slides_lang hssl ON (hss.id_homeslider_slides = hssl.id_homeslider_slides)
        ...
        foreach ($results as $result)
            $images[$result['id_lang']]['image'] = $result['image'];
            $images[$result['id_lang']]['mobile_image'] = $result['mobile_image'];
    ...
    public function getAddFieldsValues()
    {
        ...
        foreach ($languages as $lang) {
            ...
            $fields['mobile_image'][$lang['id_lang']] = Tools::getValue('mobile_image'.(int)$lang['id_lang']);
